...
def ProdolzhiPravo(head, body, zeleni, nasoka, crveni):
    dolzina = len(body)  # dolzina na teloto na zmijata
    tmp = body[-1]  # kraen del od teloto ako treba da se zgolemi snake
    body = list(body)
    t1 = [body[-1]] + body[:-1]
    head = list(head)
    t1[0] = head
    ...
    apples = zeleni
    if nasoka == "sever":
        if int(head[1]) + 1 < 10 and (head[0], int(head[1]) + 1) not in body and (
                head[0], int(head[1]) + 1) not in crveni:
            if (head[0], int(head[1]) + 1) in zeleni:
                body = list(body)
                body.append(tmp)
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (head[0], int(head[1]) + 1)]
            return (int(head[0]), int(head[1]) + 1), body, tuple(
                apples), nasoka, crveni  # glavata da a pridvizime za 1
    if nasoka == "jug":
        if int(head[1]) - 1 >= 0 and (head[0], int(head[1]) - 1) not in body and (
                head[0], int(head[1]) - 1) not in crveni:
            if (head[0], int(head[1]) - 1) in zeleni:
                body = list(body)
                body.append(tmp)
                ...
                apples = [apple for apple in zeleni if apple != (head[0], int(head[1]) - 1)]
            return (int(head[0]), int(head[1]) - 1), body, tuple(
                apples), nasoka, crveni
    if nasoka == "zapad":
        if int(head[0]) - 1 >= 0 and (int(head[0]) - 1, head[1]) not in body and (
                int(head[0]) - 1, head[1]) not in crveni:
            if (int(head[0]) - 1, head[1]) in zeleni:
                body = list(body)
                body.append(tmp)
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (int(head[0]) - 1, head[1])]
            return (int(head[0]) - 1, int(head[1])), body, tuple(apples), nasoka, crveni
    if nasoka == "istok":
        if int(head[0]) + 1 < 10 and (int(head[0]) + 1, head[1]) not in body and (
                int(head[0]) + 1, head[1]) not in crveni:
            if (int(head[0]) + 1, head[1]) in zeleni:
                body = list(body)
                body.append(tmp)
...
def SvrtiDesno(head, body, zeleni, nasoka, crveni):
    dolzina = len(body)  # dolzina na teloto na zmijata
    tmp = body[-1]  # kraen del od teloto ako treba da se zgolemi snake
    body = list(body)
    t1 = [body[-1]] + body[:-1]
    ...
    if nasoka == "sever":
        ...
        int(head[0]) + 1, head[1]) not in crveni:
            if (int(head[0]) + 1, head[1]) in zeleni:
                ...
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (int(head[0]) + 1, head[1])]
            return ((int(head[0])) + 1, int(head[1])), body, tuple(apples), "istok", crveni
    if nasoka == "jug":
        if int(head[0]) - 1 >= 0 and (int(head[0]) - 1, head[1]) not in body and (
        int(head[0]) - 1, head[1]) not in crveni:
            if (int(head[0]) - 1, head[1]) in zeleni:
                ...
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (int(head[0]) - 1, head[1])]
            return ((int(head[0])) - 1, int(head[1])), body, tuple(apples), "zapad", crveni
    if nasoka == "zapad":
        if int(head[1]) + 1 < 10 and (head[0], int(head[1]) + 1) not in body and (
        head[0], int(head[1]) + 1) not in crveni:
            if (head[0], int(head[1]) + 1) in zeleni:
                ...
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (head[0], int(head[1]) + 1)]
            return (int(head[0]), int(head[1]) + 1), body, tuple(apples), "sever", crveni
    if nasoka == "istok":
        if int(head[1]) - 1 >= 0 and (head[0], int(head[1]) - 1) not in body and (
        head[0], int(head[1]) - 1) not in crveni:
            if (head[0], int(head[1]) - 1) in zeleni:
                ...
                body = tuple(body)
                apples = [apple for apple in zeleni if apple != (head[0], int(head[1]) - 1)]
            return (int(head[0]), int(head[1]) - 1), body, tuple(apples), "jug", crveni
    return (-1, -1), (-1, -1), (-1, -1), (-1, -1), "error"


def SvrtiLevo(head, body, zeleni, nasoka, crveni):
    dolzina = len(body)  # dolzina na teloto na zmijata
    tmp = body[-1]  # kraen del od teloto ako treba da se zgolemi snake
    body = list(body)
    t1 = [body[-1]] + body[:-1]
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        novo = ProdolzhiPravo(head, body, zeleni_jabolki, nasoka, crveni_jabolki)
        if novo[0][0] != -1 and novo[0][1] != -1:
            successors['ProdolzhiPravo'] = (novo[0], novo[1], novo[2], novo[3], novo[4])
        novo = SvrtiDesno(head, body, zeleni_jabolki, nasoka, crveni_jabolki)
        if novo[0][0] != -1 and novo[0][1] != -1:
            successors['SvrtiDesno'] = (novo[0], novo[1], novo[2], novo[3], novo[4])
        novo = SvrtiLevo(head, body, zeleni_jabolki, nasoka, crveni_jabolki)
        if novo[0][0] != -1 and novo[0][1] != -1:
...
if __name__ == '__main__':
    ...
    for i in range(0, gn):
        elem = input()
        z = elem.split(",")
        dodadi = int(z[0]), int(z[1])
        zeleni_jabolki.append(dodadi)
    ...
    for i in range(0, rn):
        elem = input()
        r = elem.split(",")
        dodadi = int(r[0]), int(r[1])
        crveni_jabolki.append(dodadi)
    crveni_jabolki = tuple(crveni_jabolki)

    zmija = head, (body)
    zmija = Snake((head, body, zeleni_jabolki, orientation), crveni_jabolki)
